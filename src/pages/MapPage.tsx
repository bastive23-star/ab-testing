...
const PEEK = 52
const OPEN = typeof window !== 'undefined' ? Math.round(window.innerHeight * 0.52) : 420
const NAV = 112
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${Math.round(size * 1.22)}" viewBox="0 0 52 64">
    <defs>
      <filter id="sh" x="-50%" y="-30%" width="200%" height="200%">
        <feDropShadow dx="0" dy="4" stdDeviation="${highlighted ? 7 : 4}" flood-color="${highlighted ? 'rgba(200,48,42,0.45)' : 'rgba(0,0,0,0.22)'}"/>
      </filter>
    </defs>
...
// Separate component so we can use useMap + useEffect properly
function MapController({ flyCoords, panCoords }: {
  ...
  useEffect(() => {
    if (flyCoords) map.flyTo(flyCoords, 16, { duration: 0.7, easeLinearity: 0.5 })
  }, [flyCoords]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (panCoords) map.panTo(panCoords, { animate: true, duration: 0.45 })
  }, [panCoords]) // eslint-disable-line react-hooks/exhaustive-deps
...
export function MapPage() {
  ...
  // Sheet drag — only from handle bar
  ...
  // Scroll → highlight + debounced pan (no zoom)
  ...
  const handleScroll = useCallback(() => {
    ...
    if (!container) return
    const viewTop = container.getBoundingClientRect().top + 64
    let nearest: string | null = null
    let minDist = Infinity
    for (const r of sorted) {
      const el = itemRefs.current[r.id]
      if (!el) continue
      const dist = Math.abs(el.getBoundingClientRect().top - viewTop)
  ...
  // Tap → fly in (zoom) + collapse sheet
  ...
  return (
    <div className="relative" style={{ height: '100dvh' }}>
      {/* Map isolated in z:0 stacking context so sheet renders above leaflet panes */}
        ...
        >
          ...
          {filtered.map(r => (
            <Marker
              ...
              eventHandlers={{
                click: () => { setHighlighted(r.id); setFlyCoords(null); setPanCoords(null); snapTo(false) },
              }}
            >
      ...
      {/* Bottom Sheet — z:45, nav stays at z:50 on top */}
      <motion.div
        drag="y"
      ...
      >
        {/* Handle — only this area triggers drag */}
        <div
          className="shrink-0 cursor-grab active:cursor-grabbing select-none"
          onPointerDown={e => dragControls.start(e)}
          style={{ padding: '12px 16px 0' }}
        >
          <div className="w-9 h-[3px] rounded-full bg-[#D5D3CE] mx-auto mb-3" />
          ...
          <div className="flex items-center justify-between mt-2.5 mb-1">
            ...
            <button onClick={() => snapTo(!isOpen)} className="text-[11px] font-semibold text-[#C8302A] py-0.5">
        ...
        {/* List — scrollable, no drag conflict */}
        <div
          ref={listRef}
          ...
          {sorted.map((r, i) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'w-full flex items-center gap-4 px-5 py-3.5 text-left border-b border-[#F4F3F0] last:border-0 transition-colors duration-150',
                  active ? 'bg-[#FFF6F5]' : 'active:bg-[#F9F8F5]'
                )}
              >
                <span
                  className="font-serif tabular-nums shrink-0 leading-none"
                  style={{ fontSize: 24, color: active ? '#E8C8C6' : '#ECEAE4', width: 34, textAlign: 'right' }}
                >
                ...
                <div className="shrink-0 text-right">
                  <span
                    ...
                    style={{ fontSize: 24, color: r.avg_score > 0 ? scoreColor(r.avg_score) : '#D5D3CE' }}
                  >
                    {r.avg_score > 0 ? r.avg_score.toFixed(1) : '—'}
                  </span>
                </div>
