...
const NAV_H = 112 // nav bar + safe area
...
function MapController({ flyCoords, panCoords }: {
  ...
  const map = useMap()
  useEffect(() => { if (flyCoords) map.flyTo(flyCoords, 16, { duration: 0.7 }) }, [flyCoords]) // eslint-disable-line
  useEffect(() => { if (panCoords) map.panTo(panCoords, { animate: true, duration: 0.4 }) }, [panCoords]) // eslint-disable-line
  return null
}
...
export function MapPage() {
  ...
  // Available screen height (excludes nav)
  const avail = `calc(100dvh - ${NAV_H}px)`
  const mapH = fullscreen ? avail : `calc(${avail} * 0.42)`
  const listH = `calc(${avail} * 0.58)`

  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', paddingBottom: NAV_H }}>
      ...
      >
        ...
        <button
          ...
          aria-label={fullscreen ? 'Liste anzeigen' : 'Vollbild'}
        >
          {fullscreen ? <ListIcon /> : <ExpandIcon />}
          <span className="text-[11px] font-semibold text-[#6B6560]">
            {fullscreen ? 'Liste' : 'Vollbild'}
          </span>
        </button>

        {/* Floating filters in fullscreen */}
        <AnimatePresence>
          {fullscreen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -8 }}
              style={{ position: 'absolute', top: 14, left: 14, right: 80, zIndex: 10 }}
            ...
            >
              {[null, ...foodTypes].map(t => (
                <button
                  ...
                  className={cn(
                    'px-3 py-1.5 rounded-full text-[11px] font-semibold whitespace-nowrap shrink-0 shadow-sm backdrop-blur-sm border transition-all',
                    foodFilter === t
                      ? 'bg-[#111110] text-white border-[#111110]'
                      : 'bg-white/90 text-[#6B6560] border-[#E8E6E0]'
                  )}
                >
                  {t ?? 'Alle'}
                </button>
      ...
      <AnimatePresence>
        {!fullscreen && (
          <motion.div
            ...
            style={{ height: listH, display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}
          >
            {/* Filter + meta bar */}
            <div className="px-4 pt-3 pb-2 border-b border-[#F0EEE8] shrink-0 bg-white">
              <div className="flex gap-1.5 overflow-x-auto no-scrollbar mb-2">
                {[null, ...foodTypes].map(t => (
                  <button
                    ...
                    onClick={() => setFoodFilter(t)}
                    className={cn(
                      'px-3.5 py-1.5 rounded-full text-[11px] font-semibold whitespace-nowrap shrink-0 transition-all',
